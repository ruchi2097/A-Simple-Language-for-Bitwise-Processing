...
{
       0,    75,    75,    81,   113,   129,   135,   142,   150,   150,
     153,   154,   156,   157,   160,   168,   189,   192,   198,   199,
     205,   208,   212,   218,   223,   227,   231,   235,   239,   243,
     247,   251,   255,   259,   263,   271,   276,   283,   294,   304,
     315,   326
};
#endif
...
#line 169 "p1.y"
{
  ...
  tmp->dump();
 // ID_pair.insert({$1,Builder.getInt32(0)}); 
 // ID_pair[$1]->dump();
   auto it = myMap.find((yyvsp[-4].name));
   if (it == myMap.end()) {
     exit(1);  	
  }
  printf("ensemble \n");
  (yyvsp[-1].val)->dump(); 
  tmp = Builder.CreateOr(tmp,(yyvsp[-1].val));
  tmp->dump();
  tmp = Builder.CreateShl(tmp,(yyvsp[-3].num));
  tmp->dump();
  tmp = Builder.CreateOr(ID_pair[(yyvsp[-4].name)],tmp);
  tmp->dump();
  ID_pair[(yyvsp[-4].name)] = tmp;
  ID_pair[(yyvsp[-4].name)]->dump();
}
#line 1318 "p1.y.cpp"
    break;
